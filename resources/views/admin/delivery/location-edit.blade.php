                ...
                <div class="card-body">
                    <form action="{{ route('admin.locations.update', $location->id) }}" method="POST">
                        ...
                        <div id="sectors-wrapper">
                            ...
                            @php
                                $sectors = old('sectors', $location->sector); // $location->sector is an array of sectors
                            @endphp

                            @foreach($sectors as $index => $sector)
                                @php
                                    // Decode JSON slots
                                    $slots = !empty($sector['t_time']) ? json_decode($sector['t_time'], true) : [];
                                @endphp
                                ...
                                <div class="sector-block border p-3 mb-3">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="sectors[{{ $index }}][name]" value="{{ $sector['sector'] ?? '' }}" required>
                                    </div>

                                    <div class="form-group">
                                        <label>Delivery Taken Time</label>
                                        <input type="text" class="form-control" name="sectors[{{ $index }}][delivery_time]" value="{{ $sector['b_time'] ?? '' }}">
                                    </div>
...
<script>
    ...
    if(e.target.classList.contains('add-slot')){
        let sectorBlock = e.target.closest('.sector-block');
        let slotsDiv = sectorBlock.querySelector('.time-slots'); // fix here

        // Get the sector index
        let firstInput = sectorBlock.querySelector('input[name^="sectors"]');
        let matches = firstInput.name.match(/sectors\[(\d+)\]/);
        let sectorIndex = matches ? matches[1] : 0;

        // Create new slot input with only remove button
        let inputGroup = document.createElement('div');
        inputGroup.classList.add('input-group','mb-2');
        inputGroup.innerHTML = `
            <input type="text" class="form-control" name="sectors[${sectorIndex}][slots][]" placeholder="e.g. 10:00pm-11:00pm">
            <div class="input-group-append">
                <button type="button" class="btn btn-danger remove-slot">-</button>
            </div>`;
    ...
    if(e.target.classList.contains('remove-slot')){
        ...
        if(slotsDiv.querySelectorAll('.input-group').length > 1){
